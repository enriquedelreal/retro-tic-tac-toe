...
class RetroGameArcade {
    ...
    initializeGames() {
        try {
            ...
            if (typeof RetroPong !== 'undefined') {
                this.games.pong = new RetroPong();
                console.log('Pong initialized');
            } else {
                console.error('RetroPong not found');
            }
            
            if (typeof RetroBreakout !== 'undefined') {
                this.games.breakout = new RetroBreakout();
                console.log('Breakout initialized');
            } else {
                console.error('RetroBreakout not found');
            }
            
            if (typeof RetroPacman !== 'undefined') {
                this.games.pacman = new RetroPacman();
                console.log('Pacman initialized');
            } else {
                console.error('RetroPacman not found');
            }
            
            if (typeof RetroMinesweeper !== 'undefined') {
                this.games.minesweeper = new RetroMinesweeper();
                console.log('Minesweeper initialized');
            } else {
                console.error('RetroMinesweeper not found');
            }
    ...
    simpleSwitchGame(gameName) {
        try {
            ...
            const gameElement = document.getElementById(`${gameName}-game`);
            console.log('Looking for game element:', `${gameName}-game`);
            if (gameElement) {
                gameElement.style.display = 'block';
                gameElement.classList.add('active');
                console.log('Switched to ' + gameName);
                console.log('Game element display style:', gameElement.style.display);
                console.log('Game element classes:', gameElement.className);
                ...
                // Special handling for canvas games
                if (this.games[gameName] && typeof this.games[gameName].draw === 'function') {
                    console.log('Drawing game for:', gameName);
                    setTimeout(() => {
                        if (typeof this.games[gameName].reinitialize === 'function') {
                            this.games[gameName].reinitialize();
                        } else {
                            this.games[gameName].draw();
                        }
                    }, 100);
                }
            } else {
    ...
    // Debug function to test games
    testGames() {
        console.log('Testing games...');
        console.log('Available games:', Object.keys(this.games));
        
        // Test each game
        Object.keys(this.games).forEach(gameName => {
            const game = this.games[gameName];
            console.log(`${gameName}:`, game ? 'Initialized' : 'Not initialized');
            if (game) {
                console.log(`${gameName} methods:`, Object.getOwnPropertyNames(Object.getPrototypeOf(game)));
            }
        });
        
        // Test game elements
        const gameElements = document.querySelectorAll('.game-container');
        console.log('Game containers found:', gameElements.length);
        gameElements.forEach(element => {
            console.log('Game container:', element.id, 'display:', element.style.display, 'classes:', element.className);
        });
    }
}
...
function initializeApp() {
    ...
    const requiredClasses = [
        ...
        'RetroPong', 'RetroBreakout', 'RetroPacman', 'RetroMinesweeper'
    ];
...
window.testArcadeGames = function() {
    if (window.retroArcade) {
        window.retroArcade.testGames();
    } else {
        console.log('RetroArcade not initialized yet');
    }
}; 